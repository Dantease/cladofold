...
- The installed 1.3.0 build 9 app and the freshly packaged staging app have the same executable SHA-256: `de20c93f7ec22a929f0deae270b1f0f6292e26dbabd7b22dc15c405bbc79a654`. The build 9 DMG SHA-256 is `ad78c0c6d2b3c24fda2f8317f01bc60fa0924da35cc37656d298846d7ea50a9e`; ZIP SHA-256 is `8e5a96aca64b5b42eadf735adb561429a8d4207486cfcd9e8cf9c63fec260fe9`. Earlier draft release assets must be replaced with these packages after live acceptance.
...
The September 13 repair found live lid-angle readings but Screen Recording denied by macOS. Build 8 preserves the 30° endpoint and one-frame GPU submission limit, verifies a real capture before reporting ready, adds an upfront setup card and retry control, and rechecks on return from System Settings. The installed app's denial/retry UI was exercised: it reports a live 116–118° sensor separately from `captureState: needsPermission`, `permission: false`, `ready: false`, and no visible overlay or rendered frames. The System Settings switch is on but capture remains denied, consistent with a stale permission entry after an ad-hoc rebuild. Refreshing that entry and completing macOS authentication remain pending; successful desktop capture and physical-lid behavior have not yet been verified for build 8.

Build 9 corrects an overly broad automatic recheck added in build 8: a previous permission request no longer permits capture attempts on activation or wake. Automatic checks require a currently allowed preflight and stop after a failed capture; that stop is persisted across app launches. Only an observed new permission grant or an explicit Check again can retry. Opening Screen Recording settings no longer starts another capture check. On this Mac, launching the installed build and switching away/back left `captureChecksStarted: 0`, `preflightGranted: false`, a live 118° sensor, and no overlay or system capture request. This verifies prompt suppression in the current denied-preflight state; live desktop animation remains unverified pending permission repair. The signing identity query found no valid code-signing identities; current ad-hoc signatures remain a distribution limitation.
