...
- 25 capture recovery and prompt-loop checks: denied access, successful retry, revocation, coalesced checks, cancellation, stale completion after sleep/display changes, separate permission versus display/renderer errors, denied preflight, repeated activation/wake events, persisted retry blocking, and explicit versus automatic rechecks. Total native checks: 127.
- 34 synthetic Core Image rendering checks: progressive blur, dark borders, endpoint fidelity, deterministic reversal, 15 regression cases for unwanted darkening along the top edge, the 30-degree black endpoint, and visible blur-strength changes in both standard and Duo miniature previews. The brighter-early-fold assertion was removed because that visual change is explicitly being rolled back; the earlier mid-fold shading assertion was restored. These render generated patterns, never the desktop.
...
- The installed 1.3.1 build 12 app, the app extracted from the ZIP, and the app mounted from the read-only DMG have the same executable SHA-256: `71d2d74b7b88a93d8bdf040d5e0fe59eb53f0aed9e95707f7f35fa82427e6044`. Both packaged bundles pass strict signature verification, and the disk image passes checksum verification. The build 12 DMG SHA-256 is `ee7003dae5341dac91644a3551a3194a72f2e828a4ade5ef19ec3515e0c81136`; ZIP SHA-256 is `fc8a35d070ccf80215ed7b30789909d23d8f34729a056cf388113837886128ca`.
- App and installed System Settings pane report version 1.3.1, build 12. The previous app and preferences were backed up before installation. Every saved animation setting was compared before and after the native UI checks and is unchanged, including the owner's current 64 pt blur, 220 ms smoothing, and 30° endpoint.

## Starting-angle calibration and live appearance preview — September 13, 2026

Selecting Follow previously changed only the miniature's data source, leaving the desktop's older maximum open-angle reference intact. Build 12 explicitly calibrates the desktop and miniature to the current angle when Follow is selected, and adds Use current angle as open in settings and the menu bar. The reference stays fixed during a partial fold and hold. Returning within one sensor degree while reopening clears and starts the next cycle at the returned angle. The tests cover starting at 102° after 118°, twenty repeated fold cycles, held partial folds, and preservation of the 30° black endpoint. The display clock, smoothing response, and visual renderer from build 11 are unchanged.

Appearance sliders now update the miniature independently of whether the desktop effect is enabled. When the current preview is fully clear or black, an appearance edit shows a 55% fold so the setting is visible; an already readable partial fold is retained. Moving the physical lid while following, changing the simulated angle, or selecting Return to lid/angle resumes the angle preview. Blur, darkening, border depth, appearance toggles, and presets share this behavior.

Native UI checks with the desktop effect off and Follow selected showed the preview change from sharp at 0 pt to blurred at 80 pt. Intermediate native slider actions updated its rendered image and accessible value immediately. The owner's 64 pt setting was restored, and Return to lid restored a clear preview. Selecting Follow set both the live runtime reference and miniature to this Mac's current 107° angle. Physical 102° closing/reopening acceptance remains pending.

The rebuilt ad-hoc signature again encountered a stale screen-access entry. Re-adding it alone did not restore capture, and macOS disabled Remove for that entry. Under the owner's existing permission-repair authorization, `tccutil reset ScreenCapture com.dante.Foldable` cleared only this app's screen-access records. The installed copy was then added in System Settings. One explicit Check again verified real ScreenCaptureKit capture. A subsequent normal quit/relaunch retained verified access without another prompt.

The installed six-second desktop preview reached full effect and returned to zero with its overlay hidden. Initial capture took 25.1 ms; the final sampled callback cadence was 118.6 Hz, with p95 interval 8.76 ms and maximum 15.3 ms. These are callback timings on one Mac, not an end-to-end display-presentation or physical-lid benchmark. The public download remains 1.3.0 build 9 until the owner accepts this candidate's physical motion.
